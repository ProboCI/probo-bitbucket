...
<<<<<<< HEAD
  ...
  // check this.token for token, give it a try, if not refresh the token using this.oauth


=======
  var method = opts.method || "get";
>>>>>>> Return a new token via auth_lookup.
  ...
  return performRequest(method, opts, handler);
};

Bitbucket.prototype.performRequest = function(method, opts, handler) {
  var self = this;
  var cb = function(error, response, body) {
    ...
    if (self.isTokenError(body)) {
      return self.retryRequestWithFreshToken(method, opts, handler);
    }
...
Bitbucket.prototype.retryRequestWithFreshToken = function(method, opts, handler, attempts) {
  ...
  self.getNewToken(opts, function(opts) {
...
Bitbucket.prototype.getNewToken = function(opts, done) {
  var self = this;
  request.get(this.refreshOAuth, function(error, response, body) {
    if (error) {
      console.log(error);
      process.exit();
    }
    self.token = body.access_token;
    opts.headers = self.makeAuthHeader(),
    done(opts);
  });
}
...
Bitbucket.prototype.makeAuthHeader = function() {
