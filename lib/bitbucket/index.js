...
/**
 * @param {Object} config = An object containing configuraiton options.
 * @param {Object} config.log - The logger to use.
 * @param {String} config.version -the Bitbucket API version to use. Defaults to '2.0'
 * @param {Object} config.auth - An option contianing bitbucket auth data.
 * @param {String} config.auth.consumerKey - The consumer key for the API (get from BitBucket integration dashboard).
 * @param {String} config.auth.consumerSecret - The consumer secret for the API (get from BitBucket integration dashboard).
 * @param {String} config.auth.token - The token that was generated by BitBucket for the user.
  ...
  for (let i in subs) {
    if (subs.hasOwnProperty(i)) {
      ...
      for (let m in this[sub]) {
        if (this[sub].hasOwnProperty(sub)) {
          method = this[sub][m];
