...
/**
 * Container Manager mode has no coordinator to persist to — tokens come from
 * static config. Keep the refreshed token on the handler so the rest of this
 * process's requests use it instead of the stale one from the config file.
 *
 * @param {Object} project - The project whose tokens were refreshed. Unused here.
 * @param {Object} tokens - The token payload returned by Bitbucket.
 * @param {Function} [cb] - The callback to call when completed.
 * @return {Null} - No return value.
 */
CMAPI.prototype.updateTokens = function(project, tokens, cb) {
  if (tokens && tokens.access_token && this.handler) {
    this.handler.bbAccessToken = tokens.access_token;

    if (tokens.refresh_token) {
      this.handler.bbRefreshToken = tokens.refresh_token;
    }
  }

  this.log.debug('Container Manager mode: refreshed Bitbucket tokens kept in memory only');
