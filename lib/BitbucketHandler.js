...
class BitbucketHandler {
  ...
  hashBuildController(req, res, next) {
    ...
    this.bitbucket.getCommit(project, sha, (err, commit) => {
      // If the commit message does not have [build] in it, then do nothing.
      if (commit.message.indexOf('[build]') === -1) {
        res.send(501, {});
        return next();
      }
  ...
  pullRequestHandler(event, cb) {
    ...
    let request = {
      ...
      owner:
        event.payload.repository.owner.username ||
        event.payload.repository.full_name.split('/')[0],
  ...
  buildStatusUpdateHandler(update, build, cb) {
    ...
    const task = this.bitbucket.postStatus.bind(
      this.bitbucket,
      build.project,
      build.commit.ref,
      statusInfo
    ...
    statusUpdateQueue.push(task, (error) => {
      if (error) {
        this.logger.error(
          { err: error, build: build },
          'An error occurred posting status to Bitbucket'
        );
      ...
      this.logger.info(
        statusInfo,
        'Posted status to Bitbucket for',
        build.project.slug,
        build.commit.ref
      );
      cb(null, statusInfo);
  ...
  processBuild(project, request, cb) {
    this.bitbucket.fetchProboYamlConfig(
      project,
      request.sha,
      (error, config) => {
        var build;
  ...
  submitBuild(request, project, config, cb) {
    ...
    this.api.submitBuild(build, project, (err, submittedBuild) => {
      if (err) {
        ...
        this.logger.error(
          { err: err, request: request, build: build, message: submittedBuild },
  ...
  authLookupController(req, res, next) {
    ...
    try {
      ...
      this.logger.error(
        { err: e },
        'Problem getting auth header: ' + e.message
      );
