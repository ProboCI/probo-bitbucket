...
var BitbucketHandler = function(options) {
  ...
  var pullRequestController = function(req, res, next) {
    var api = self.getBitbucketApi({service_auth: req.query});
    var opts = {
      owner: req.params.owner,
      repo: req.params.repo,
      id: req.params.pullRequest,
      authToken: req.query.token,
      refreshToken: req.query.refreshToken,
    };
    api.repos.getPullRequest(opts, function(error, prData) {
      res.json(prData);
    });
  };
  server.get('/pull-request/:owner/:repo/:pullRequest', pullRequestController);
