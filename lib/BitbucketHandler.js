...
var WebhookHandler = require('./BitbucketWebhook');

var Bitbucket = require('./bitbucket/BitbucketApi');
...
var BitbucketHandler = function(options) {
  ...
  this.pullRequestClosedHandler = this.pullRequestClosedHandler.bind(this);
  ...
  var logLevel = options.log_level || 'debug';
  var loggerOptions = {
  ...
  if (options.logFile && options.logFile.enabled && options.logFile.path) {
    var logPath = path.resolve(options.logFile.path);
    var logDir = path.dirname(logPath);
    if (!fs.existsSync(logDir)) {
      fs.mkdirSync(logDir, {recursive: true});
    }
    // Providing `streams` replaces bunyan's default stdout stream,
    // so nothing is written to stdout/stderr when file logging is on.
    loggerOptions.streams = [{
      type: 'rotating-file',
      path: logPath,
      period: options.logFile.period || '1d',
      count: options.logFile.count != null ? options.logFile.count : 7,
      level: logLevel,
    }];
  }
  ...
  // A merged PR fires pullrequest:fulfilled; a declined PR fires
  // pullrequest:rejected. Both mean the PR is closed, so we notify the
  // Container Manager to reap its builds (including in-progress ones).
  handler.on('pullrequest:fulfilled', this.pullRequestClosedHandler);
  handler.on('pullrequest:rejected', this.pullRequestClosedHandler);
  ...
  // verbatim
  server.post(handlerOptions.path, restify.plugins.bodyParser(), function(req, res, next) {
  ...
  server.post('/builds/:bid/status/:context', restify.plugins.jsonBodyParser(), buildStatusController);
  server.post('/update', restify.plugins.jsonBodyParser(), buildStatusController);
  server.post('/builds/hash', restify.plugins.jsonBodyParser(), this.buildHashController.bind(this));

  server.get('/pull-request/:owner/:repo/:pullRequestNumber', this.pullRequestController.bind(this));
  server.get('/branches/:owner/:repo', this.branchesController.bind(this));
  server.get('/commits/:owner/:repo', this.commitsController.bind(this));
...
BitbucketHandler.prototype.getBitbucketApi = function(project) {
  ...
  if (!project.service_auth || (!project.service_auth.token && !project.service_auth.refreshToken)) {
    throw new Error('Project is missing Bitbucket OAuth credentials (service_auth). Re-sync the repository to restore access.');
  }
  ...
  var bitbucket = new Bitbucket({
    ...
    onTokenRefreshed: function(newToken) {
      // Update the project's service_auth so subsequent API calls
      // for this project use the refreshed token, not a stale one.
      project.service_auth.token = newToken.access_token;
      if (newToken.refresh_token) {
        project.service_auth.refreshToken = newToken.refresh_token;
      }

      // Write the new tokens back to the coordinator so the next webhook starts
      // with a valid token instead of burning a request to discover it expired.
      // Fire and forget: failing to persist shouldn't fail the build in flight.
      self.api.updateTokens(project, newToken, function(err) {
        if (err) {
          self.log.error({err: err, projectId: project.id}, 'Unable to persist refreshed Bitbucket tokens');
        }
      });
    },
  });
...
/**
 * Handles a merged (pullrequest:fulfilled) or declined (pullrequest:rejected)
 * pull request by notifying the Container Manager so the reaper can clean up
 * builds for the branch, including any build still in progress when the PR was
 * closed.
 *
 * @param {Object} event - The Bitbucket webhook event.
 * @param {Function} cb - The callback to call when completed.
 */
BitbucketHandler.prototype.pullRequestClosedHandler = function(event, cb) {
  var self = this;
  var pr = event.payload.pullrequest;

  this.log.info('Bitbucket pull request ' + pr.id + ' for ' +
                event.payload.repository.full_name + ' closed.');
  ...
  this.api.findProjectByRepo(request, function(error, project) {
    if (error || !project) {
      self.log.info({error: error},
        'Project for Bitbucket repo ' + request.slug + ' not found, cannot process PR close');
      return cb && cb(error);
    }

    var branchName = pr.source.branch.name;
    self.api.notifyPrClosed(project.id, branchName, function(error, result) {
      self.log.info({projectId: project.id, branchName: branchName, err: error, result: result},
        'PR closed event processed');
...
BitbucketHandler.prototype.processRequest = function(request, cb) {
  ...
  this.api.findProjectByRepo(request, function(error, project) {
    if (error || !project) {
      error = error || new Error('Project for bitbucket repo ' + request.slug + ' not found');
      self.log.warn(
        {err: error},
    ...
    self.fetchProboYamlConfigFromBitbucket(project, request.sha, function(error, config) {
      ...
      if (error) {
        ...
        var update = {
          ...
          context: 'ProboCI/env',
          // Bitbucket rejects a build status without a url, so the failure
          // would never reach the PR. There's no build to link to yet, so
          // point at the PR (or the commit) instead.
          target_url: request.pull_request_html_url || request.commit_url,
        };

        // Report the failure on the commit, then propagate the original error:
        // a posted red status is not a submitted build, and callers rely on
        // that distinction to know whether the commit was actually built.
        return self.buildStatusUpdateHandler(update, build, function(statusError) {
          if (statusError) {
            self.log.error({err: statusError, sha: request.sha}, 'Unable to post config failure status to Bitbucket');
          }
...
BitbucketHandler.prototype.branchesController = function(req, res, next) {
  var bitbucket = this.getBitbucketApi({
    service_auth: {refreshToken: req.query.refreshToken},
  });
  var opts = {
    owner: req.params.owner,
    repo: req.params.repo,
  };
  bitbucket.repos.getBranches(opts, function(error, branches) {
    if (error) {
      req.log.error({error, opts});
      return res.json(500, {message: 'Failed to fetch branches.', error: error});
    }
...
BitbucketHandler.prototype.commitsController = function(req, res, next) {
  var bitbucket = this.getBitbucketApi({
    service_auth: {refreshToken: req.query.refreshToken},
  });
  var opts = {
  ...
  bitbucket.repos.listCommits(opts, function(error, commits) {
    if (error) {
      req.log.error({error, opts});
      return res.json(500, {message: 'Failed to fetch commits.', error: error});
    }
...
/**
 * Triggers a build from a specific commit hash.
 * Expects the coordinator to POST { project, sha } — the full project object
 * is used directly, skipping the redundant findProjectByRepo lookup.
 *
 * @param {Object} req - The request object. Body must contain project and sha.
 * @param {Object} res - The response object.
 * @param {Function} next - The next middleware to run.
 */
BitbucketHandler.prototype.buildHashController = function(req, res, next) {
  var self = this;
  var body = req.body;

  try {
    this._validate(body, ['sha', 'project'], 'Missing required build hash param: ');
  }
  catch (e) {
    res.send(400, {error: e.message});
    return next();
  ...
  if (!project.owner && project.id) {
    project.owner = project.id.split('-')[0];
  }
  ...
  // Fetch commit info to get commit message and URL for the build name.
  bitbucket.repos.getCommit({owner: project.owner, repo: project.repo, ref: sha}, function(err, commit) {
    if (err) {
      self.log.error({err: err}, 'Problem getting commit info.');
      res.send(500, {error: err.message});
      return next();
    }

    var commitUrl = commit.links.html.href;
    var commitMessage = commit.message;

    self.fetchProboYamlConfigFromBitbucket(project, sha, function(error, config) {
      if (error) {
        self.log.error({err: error}, 'Problem fetching Probo Yaml Config file');
        var update = {
          state: 'failure',
          description: error.message,
          context: 'ProboCI/env',
          // Bitbucket rejects a build status without a url; there's no build
          // to link to yet, so point at the commit.
          target_url: commitUrl,
        };
        var errBuild = {commit: {ref: sha}, project: project};
        return self.buildStatusUpdateHandler(update, errBuild, function() {
      ...
      self.api.submitBuild(build, project, function(err, submittedBuild) {
        if (err) {
          self.log.error({err: err, response: submittedBuild, build: build}, 'Problem submitting build');
          res.json(400, {error: err.message});
          return next();
        }
        self.log.info({build: submittedBuild}, 'Submitted hash build');
        res.json(200, submittedBuild);
        next();
      });
    });
  });
