...
class BitbucketHandler {
  ...
  constructor(config) {
    ...
    this.bitbucket = new Bitbucket(this.config, this.log);
    ...
    this.server = restify.createServer({log: this.log, name: 'Probo Bitbucket'});
    ...
    if (!(this.api instanceof API)) {
      this.log.info('api.token not found, using Container Manager API directly');
    }
  }
  ...
  start(cb) {
    this.server.listen({port: this.config.port, host: this.config.hostname || '0.0.0.0'}, () => {
      this.log.info('Now listening on', this.server.url);
  ...
  _setupServer() {
    ...
    this.server.on('uncaughtException', (request, response, route, error) => {
      this.log.error({err: error, route: route}, 'Uncaught Exception');
      response.send(error);
    });
  }
    ...
    this.server.post(this.webhookOptions.path, restify.plugins.bodyParser(), (req, res, next) => {
      this.handler.handle(req, res, error => {
        if (error) {
          ...
          this.log.error({err: error}, 'Error processing hook');
        }
  ...
  buildStatusController(req, res, next) {
    ...
    this.log.debug({payload: payload}, 'Update payload');
  ...
  pullRequestHandler(event, cb) {
    this.log.info('Bitbucket Pull request ' + event.payload.pullrequest.id +
    ...
    this.processPullRequest(request, (error, build) => {
      this.log.info({type: request.type, slug: request.slug, err: error}, 'request processed');
  ...
  buildStatusUpdateHandler(update, build, cb) {
    this.log.info({update: update, build_id: build.id}, 'Got build status update');
    ...
    const task = this.bitbucket.postStatusToBitbucket.bind(this.bitbucket, build.project, build.commit.ref, statusInfo);
    statusUpdateQueue.push(task, error => {
      if (error) {
        this.log.error({err: error, build_id: build.id}, 'An error occurred posting status to Bitbucket');
        return cb(error, statusInfo);
      }

      this.log.info(statusInfo, 'Posted status to Bitbucket for', build.project.slug, build.commit.ref);
      cb(null, statusInfo);
    });
  }
  ...
  processPullRequest(request, cb) {
    this.log.info({type: request.type, id: request.id}, 'Processing request');
    ...
    this.api.findProjectByRepo(request, (error, project) => {
      ...
      this.bitbucket.fetchProboYamlConfig(project, request.sha, (error, config) => {
        ...
        if (error) {
          this.log.error({err: error}, 'Problem fetching Probo Yaml Config file');
        ...
        this.log.info({config: config}, 'Probo Yaml Config file');
  ...
  submitBuild(request, project, config, cb) {
    ...
    this.api.submitBuild(build, project, (err, submittedBuild) => {
      if (err) {
        ...
        this.log.error({err: err, request: request, build: build, message: submittedBuild}, 'Problem submitting build');
        return cb && cb(err);
      }

      this.log.info({build: submittedBuild}, 'Submitted build');
