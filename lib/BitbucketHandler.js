 ...
 * params:
 *  - type: 'bitbucket'
 *  - provider_slug: provider slug (will always be 'bitbucket')
 *  - refreshToken: user's refresh token
 */
BitbucketHandler.prototype.authLookupController = function(req, res, next) {
  this.log.debug({query: req.query}, `auth lookup request: ${req.query.provider_slug}`);

  try {
    // validate query params
    this._validate(req.query, ['provider_slug', 'refreshToken'],
