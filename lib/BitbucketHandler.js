...
BitbucketHandler.prototype.buildHashController = function(req, res, next) {
  ...
  var slug = project.slug || (project.owner + '/' + project.repo);
  var branch = body.branch || 'unknown';
  var repoHtmlUrl = 'https://bitbucket.org/' + slug;

  var request = {
    type: 'hash',
    service: 'bitbucket',
    branch: branch,
    branch_html_url: repoHtmlUrl + '/branch/' + branch,
    owner: project.owner,
    repo: project.repo,
    slug: slug,
    sha: sha,
    commit_url: repoHtmlUrl + '/commits/' + sha,
  };

  self.fetchProboYamlConfigFromBitbucket(project, sha, function(error, config) {
    if (error) {
      self.log.error({err: error}, 'Problem fetching Probo Yaml Config file');
      // If service_auth is missing we cannot post status to Bitbucket — skip
      // the status update and return the error directly.
      if (!project.service_auth || !project.service_auth.token) {
    ...
    self.api.submitBuild(build, project, function(err, submittedBuild) {
      if (err) {
        self.log.error({err: err, response: submittedBuild, request: request, build: build}, 'Problem submitting build');
        res.json(400, {error: err.message});
        return next();
      }
