...
class Bitbucket {
  ...
  /**
   * Gets the list of branches for a Bitbucket repo.
   *
   * @param {Object.<string, any>} query - Query parameters.
   * @param {string} query.owner - The repo owner/workspace.
   * @param {string} query.repo - The repo slug.
   * @param {string} query.refreshToken - The OAuth refresh token for auth.
   * @return {Promise<Array>} - Resolves to array of branch objects.
   */
  getBranches(query) {
    const bitbucket = this.getApi({service_auth: {refreshToken: query.refreshToken}});

    return new Promise((resolve, reject) => {
      bitbucket.repos.getBranches({owner: query.owner, repo: query.repo}, (err, branches) => {
  ...
  /**
   * Gets the list of commits for a Bitbucket repo.
   *
   * @param {Object.<string, any>} query - Query parameters.
   * @param {string} query.owner - The repo owner/workspace.
   * @param {string} query.repo - The repo slug.
   * @param {string} query.refreshToken - The OAuth refresh token for auth.
   * @param {string} [query.sha] - Branch name or commit SHA to filter by (optional).
   * @return {Promise<Array>} - Resolves to array of commit objects.
   */
  getCommits(query) {
    const bitbucket = this.getApi({service_auth: {refreshToken: query.refreshToken}});

    return new Promise((resolve, reject) => {
      bitbucket.repos.listCommits({owner: query.owner, repo: query.repo, sha: query.sha}, (err, commits) => {
        if (err) return reject(err);
        resolve(commits);
      });
    });
